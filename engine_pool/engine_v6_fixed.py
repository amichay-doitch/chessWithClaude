...
class ChessEngine:
        ...
        for color in [chess.WHITE, chess.BLACK]:
            ...
            for knight_sq in board.pieces(chess.KNIGHT, color):
                if knight_sq in corners:
                    # Count mobility (v6.1 fix: use attacks() instead of legal_moves)
    ...
    def negamax(self, board: chess.Board, depth: int, alpha: int, beta: int, ply: int, do_null: bool = True) -> int:
        ...
        # v6.1 fix: Check more frequently and add safety margin
        if self.time_limit and self.nodes_searched % 2048 == 0:  # Was 4096
            if time.time() - self.start_time > self.time_limit * 0.95:  # 95% safety margin
                self.time_exceeded = True
                return 0
        ...
        # Check extension (v6.1 fix): Reduced limit to prevent depth explosion
        if in_check and depth < self.max_depth + 2:  # Was +5, now +2 for safety
            depth += 1
    ...
    def search(self, board: chess.Board, depth: int = None) -> SearchResult:
        ...
        for current_depth in range(1, depth + 1):
            ...
            window_failed = False  # Track if window failed (v6.1 fix)
            ...
            for i, move in enumerate(moves):
                ...
                # Track if aspiration window failed (v6.1 fix: don't re-search here)
                if current_depth >= 5 and not window_failed and (score <= alpha or score >= beta):
                    window_failed = True  # Mark that we need to re-search with full window
            ...
            # Re-search with full window if aspiration failed (v6.1 fix)
            if window_failed and not self.time_exceeded:
                alpha = -self.INFINITY
                beta = self.INFINITY
                current_best_move = None
                current_best_score = -self.INFINITY
